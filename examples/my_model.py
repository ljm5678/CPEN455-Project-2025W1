...
def bayes_inverse_llm_classifier(args, model, batch, tokenizer, device):
    ...
    prompts_ham = [
        get_prompt(subject=subj, message=msg, label=ENRON_LABEL_INDEX_MAP.inv[0], max_seq_length=args.max_seq_len,
                   user_prompt=args.user_prompt) for subj, msg in zip(subjects, messages)]

    prompts_spam = [
        get_prompt(subject=subj, message=msg, label=ENRON_LABEL_INDEX_MAP.inv[1], max_seq_length=args.max_seq_len,
                   user_prompt=args.user_prompt) for subj, msg in zip(subjects, messages)]
...
def train_or_test(args, model, tokenizer, batch, optimizer=None, is_training=True):
    ...
    if -1 in label_indexs:
        ...
        prompts = [get_prompt(subject=subj, message=msg, label=label, max_seq_length=args.max_seq_len) for
                   subj, msg, label in zip(subjects, messages, labels_text)]
...
if __name__ == "__main__":
    ...
    training_dataset, val_dataset = prepare_subset(train_n_val_dataset, int(0.8 * len(train_n_val_dataset)),
                                                   ratio_spam=0.5, return_remaining=True)
    ...
    for iteration in tqdm(range(args.num_iterations), desc="Training"):
        ...
        if (iteration + 1) % 5 == 0:
            ...
            with torch.no_grad():
                ...
                bpd = val_bpd_logger.compute_average()
                acc = val_acc_logger.compute_accuracy()

                if bpd >= min_val_bpd:
                    if patience_counter >= patience:
                        early_stop = True
                        print("Early stopping triggered. previous min_val_bpd:", min_val_bpd, "current bpd:", bpd, "acc:", acc)
                        ...
                        torch.save(
                        {
                                ...
                                "min_val_bpd": min_val_bpd,
                            },
                            os.path.join(os.getcwd(), "ckpt", "bayes_inverse_finetuned_model.pt")
